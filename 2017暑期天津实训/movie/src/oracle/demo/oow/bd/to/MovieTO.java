 ...
 * belong to.
 * <p>
 * All the above attributes of the movies can be accessed by simple
 * getter/setter methods but in addition movie data can also be serialized into
 * JSON text, which can be saved in kv-store and then can be retrieved from
 * there to be later converted into the MovieTO by simply passing the JSON text
...
public class MovieTO extends BaseTO implements Comparable<MovieTO>
{
	...
	/**
	 * Each movie suppose to have movie poster. This attribute is used to save
	 * the poster URL
	 **/
	 ...
	 * number of votes this movie has, userRating represent rating given by the
	 * customer for this movie.
	 **/
	...
	/**
	 * This constructor takes JSON text as an input argument and constructs JSON
	 * object from this string object. Once JSON object is constructed values
	 * are read and set into MovieTO object.
	 * 
	 * @param movieJsonTxt
	 */
	...
	/**
	 * If you have JSON object (instead of JSON text) then you can simply pass
	 * to this constructor that will then execute getter operation to extract
	 * values for different parameters.
	 * 
	 * @param objectNode
	 *            - JSON representation of MovieTO
	 */
	...
	public double getPrice()
	{
		...
		{
			System.out.println(
					"ERROR: Release date " + this.getDate() + " for movie " + this.getTitle() + " is not a valid.");
		}
	 ...
	 *
	 * @param genreTO
	 *            defining one of the genre this movie belongs to.
	 */
	...
	/**
	 * Get CastCrewTO object that has information about all the cast and crew
	 * for this movie
	 **/
	...
	/**
	 * This method compared this movie object with the other movie object passed
	 * as an attribute and rank the movie based on the order value in dessending
	 * order
	 **/
	 ...
	 * from the JSON object and passed into the MovieTO object for simple data
	...
	public void setMovieJson(ObjectNode objectNode)
	{
		...
		try
		{
			...
			Iterator<JsonNode> iter = objectNode.get(JsonConstant.GENRES).iterator();
			...
			while (iter.hasNext())
			{
				...
				// System.out.println("Id: " + genreTO.getId() + " Name: " +
				// genreTO.getName());
		...
		/*
		 * 
		 * System.out.println("ID: " + id + " Adult: " + adult + " Title: " +
		 * title + " Runtime: " + runTime + " voteAvg: " + voteAvg +
	...
	public String toString()
	{
		...
		String movieStr = "" + this.getId() + Constant.DELIMITER + this.title + Constant.DELIMITER + this.date
				+ Constant.DELIMITER + this.popularity + Constant.DELIMITER + this.voteCount + Constant.DELIMITER
				+ description;
