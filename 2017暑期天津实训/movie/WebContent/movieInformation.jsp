...
<%
	...
	if (movieId != null)
	{
		...
		//insert user action to activity
		aDAO.insertCustomerActivity(activityTO2);

		MovieDAO movieDAO = new MovieDAO();
		//get movie infomation by id
		...
		if (actors != null)
		{
			for (int i = 0; i < actors.size(); i++)
			{
				...
				if (i < 6)
				{
					shortActors = shortActors + "<a type='actor' href=" + act.getId() + "> " + act.getName() + "</a>, ";
				}
			}
		}
		if (crews != null)
		{
			for (int i = 0; i < crews.size(); i++)
			{
				...
				if (c.getJob().equalsIgnoreCase("Director"))
				{
					director = director + "<a type='crew' href=" + c.getId() + "> " + c.getName() + "</a>, ";
				}
				if (c.getJob().equalsIgnoreCase("Writer")) 
				{
					writer = writer + "<a type='crew' href=" + c.getId() + "> " + c.getName() + "</a>, ";
				}
				cast = cast + c.getName() + ", ";
			}
		}
		...
		if (!shortActors.equalsIgnoreCase(""))
		{
			shortActors = shortActors.substring(0, shortActors.length() - 2);
		}
%>
			...
			<div class="leftInfoPanel">
				...
				<div class="videoContainer" style="position: relative;">
					...
					<div class="videoContainer">
						...
						<br> <a href="#movie" class="playButton"
							movieid="<%=movieId%>"></a>
					</div>
					<div>
						<%
